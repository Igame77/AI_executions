...
def quality(req: QualityRequest) -> QualityResponse:
    ...
    if ok_for_model:
        logging.info('check quality. Status: ok')
        message = "Данных достаточно, модель можно обучать (по текущим эвристикам)."
    else:
        logging.info('check quality. Status: low quality')
...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    try:
        ...
        logging.error(f'error read csv: {exc}')
...
@app.post(
        '/flags-from-csv',
        tags=['quality'],
        summary='Получение флагов эвристик из csv-файла'
)
async def quality_flags_from_csv(file: UploadFile = File(...)) :
    try:
        ...
        logging.error(f'Error read csv {exc}')  # noqa: BLE001
        raise HTTPException(status_code=400, detail=f"Не удалось прочитать CSV: {exc}")
    ...
    quality_flags = {key: bool(quality_flags[key]) for key in quality_flags if type(quality_flags[key]) in [np.bool_, bool]}

    logging.info(quality_flags)
